...
3. Read [model selection](references/model-selection.md). Compare candidates by mathematical structure, data conditions, objective, interpretability, scale, and uncertainty.
...
Return a `MODEL SUMMARY` with: problem/objective; variables/units; assumptions; baseline; main model/equations; algorithm/parameters; validation performed; sensitivity/robustness performed; saved result artifacts; key results; limitations. Use [modeling plan](templates/modeling-plan.md), [model comparison](templates/model-comparison.md), or [experiment plan](templates/experiment-plan.md) as needed.
