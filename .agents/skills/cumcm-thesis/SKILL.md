...
2. Choose the paper structure from the actual question dependency graph using [paper structure](references/paper-structure.md). Do not narrate the chronological work log.
3. For the abstract and problem analysis, read [abstract and analysis](references/abstract-and-analysis.md). Follow the reasoning chain: problem nature -> evidence/research basis -> candidate method -> justified simplification -> targeted improvement -> verified result. Vary prose naturally; do not force a generic data-cleaning/prediction/optimization/robustness sequence.
4. For assumptions, notation and equations, read [assumptions, notation and model](references/assumptions-notation-model.md).
5. Explain results rather than reciting tables. Read [results and evaluation](references/results-and-evaluation.md) for scope-specific strengths, limitations and improvements.
6. Use [LaTeX style](references/latex-style.md) for directly insertable mathematics and [academic style](references/academic-style.md) for concise prose.
7. Run [review checklist](templates/review-checklist.md) and record failed/unverified items. Check [common errors](references/common-errors.md).
...
Produce the requested section plus a compact review ledger: claim, source artifact/key, evidence state, unit/symbol check, and unresolved action. Use [paper outline](templates/paper-outline.md), [abstract template](templates/abstract-template.md), or [section template](templates/section-template.md) only as shape guides; adapt them to the actual question graph.
